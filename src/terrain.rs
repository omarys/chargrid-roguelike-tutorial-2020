...
impl Room {
    ...
    // Returns a coord at the center of the room, rounding down
    ...
    // Returns true if an only if each cell of 'grid' overlapping this room is 'None'
...
pub fn generate_dungeon<R: Rng>(size: Size, rng: &mut R) -> Grid<TerrainTile> {
    ...
    // Attempt to add a room a const number of times
    ...
    for _ in 0..NUM_ATTEMPTS {
        ...
        if room.only_intersects_empty(&grid) {
            ...
            // Add the player to the center of the room if it's the first room
            if !player_placed {
